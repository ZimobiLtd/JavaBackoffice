     ...
     */
        ...
        throws ServletException, IOException {
            ...
            try 
            {
                   ...
                   if(function.equals("getDormantPlayers"))
                   {
                        ...
                        responseobj=getDormantPlayer("2017-01-23" ,"2017-01-30");
                   }
        ...
        public JSONArray getDormantPlayer(String from,String to)
        {
            ...
            String dataQuery = "SELECT msisdn,NAME,email,registration_date,Player_Balance,Bonus_Balance, "
                    + "(SELECT ifnull(max(Play_Bet_Timestamp),'0') FROM player_bets WHERE Play_Bet_Mobile=msisdn and Play_Bet_Timestamp not between '" + from + "' and '" + to + "' GROUP BY  Play_Bet_Mobile)"
                    + "FROM player "
                             + "WHERE msisdn NOT  IN ( SELECT Play_Bet_Mobile FROM player_bets  where Play_Bet_Timestamp between '" + from + "' and '" + to + "' ) and NAME is not null GROUP BY msisdn  limit 100  ";
